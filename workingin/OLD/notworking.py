...
class Interface:
    ...
    def gui(self):
        self.window = Tk()
        self.window.title("QueueLol")
        self.window.geometry("350x300")
        self.window.iconphoto(False, PhotoImage(file='logo.png'))
        self.acc = IntVar()
        self.pick = IntVar()  

        top_label = Label(self.window, width=400, height=1, text=("==Qu=uu=e==e=lo==l=l==="), font=("Arial 15 bold"), fg="white", bg="#429ef5")
        top_label.pack()

        autoacc_label = Label(self.window, width=8, height=2, text="Autoacc", font=("Arial 15"))
        autoacc_label.pack(side=LEFT, anchor=NW)

        autoacc_button = Checkbutton(self.window, height=3, variable=self.acc, command=self.manager_acc)
        autoacc_button.pack(side=LEFT, anchor=NW)

        self.autopick_label = Label(self.window, width=8, height=2, text="Autopick", font=("Arial 15"))
        self.autopick_label.pack(side=LEFT, anchor=NW)

        autopick_button = Checkbutton(self.window, height=3, variable=self.pick, command=self.manager_pick)
